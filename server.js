...
app.get('/api/habits/:id', (req, res) => {
  ...
  try {
    const habit = habitsDb.getById(parseInt(id));
...
app.post('/api/habits', (req, res) => {
  ...
  try {
    const newHabit = habitsDb.create(title.trim(), description.trim());
...
app.put('/api/habits/:id', (req, res) => {
  ...
  try {
    ...
    const existingHabit = habitsDb.getById(parseInt(id));
    if (!existingHabit) {
      return res.status(404).json({ error: 'Habit not found' });
    }

    // Update the habit
    const updated = habitsDb.update(parseInt(id), title.trim(), description.trim());
    
    if (updated) {
      const updatedHabit = habitsDb.getById(parseInt(id));
...
app.delete('/api/habits/:id', (req, res) => {
  ...
  try {
    ...
    // Delete the habit
    const deleted = habitsDb.delete(parseInt(id));
